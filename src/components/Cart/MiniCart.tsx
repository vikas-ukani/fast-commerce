...
export const MiniCart = ({ showMiniCart, setShowMiniCart }: {
  showMiniCart: boolean;
  setShowMiniCart: (flag: boolean) => void;
}) => {
  const { token } = useSelector((state: IRootState) => state.auth);
  const { items: cartItems } = useSelector((state: IRootState) => state.cart);
  const dispatch = useDispatch();
  const { addToCart, removeItemFromCart } = useCart();
  const [loading, setLoading] = useState(false);

  const handleQuantity = async (cartItemId: string, productId: string, quantity: number) => {
    console.log('cartItemId, productId, quantity :>> ', cartItemId, productId, quantity);
    if (loading) return;
    ...
    try {
      ...
      setTimeout(() => {
        if (cartItems.length == 0) setShowMiniCart(false)
      }, 1000);
    } catch (error) {
