    ...
    return (
        <div className='space-y-6 '>
            {products.length > 0 && products?.map((product, idx) => (
                <div key={idx} className="flex min-h-80 font-sans w-full bg-gray-100 shadow-md rounded-xl hover:shadow-lg hover:scale-100 border hover:border-primary duration-300">
                    <div className="flex-none w-80 relative">
                        <Link href={`/products/${product._id}`}>
                            <Image src={product.thumbnail} alt={product.title} className="absolute inset-0 w-full h-full object-cover rounded-l-xl" loading="lazy" height={300} width={300} />
